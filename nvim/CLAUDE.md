...
### 2026-06-07: Neovim 0.12 対応 & ディレクトリ構造修正
- **問題1**: `~/.config/nvim/lua/` が存在せず、`init.lua` の `require('core.lazy')` が `E5113: module not found` で失敗していた
- **修正1**: `nvim/lua/init.lua` と `nvim/lua/lazy-lock.json` を `nvim/` 直下に移動。`default.nix` で `programs.neovim.initLua = builtins.readFile ./init.lua;` と `xdg.configFile."nvim/lua".source = ./lua;` を使う構成に変更
- **問題2**: Neovim 0.12 で `programs.neovim.extraLuaConfig` が `initLua` にリネーム
- **修正2**: `extraLuaConfig` → `initLua` に変更
- **問題3**: `vim.lsp.semantic_tokens.enable(false, { bufnr = ..., client_id = ... })` で `bufnr` と `client_id` が排他的になった
- **修正3**: `vim.lsp.semantic_tokens.enable(false, { bufnr = args.buf })` のみに変更 (バッファ単位)
- **問題4**: `client.server_capabilities.semanticTokensProvider = nil` のような直接代入は古いパターン
- **修正4**: `client:supports_method("textDocument/inlayHint")` を使うように変更
- **問題5**: `vim.keymap.set` の `buffer` キーは 0.12 で `buf` にリネーム (deprecated)
- **修正5**: `{ buffer = args.buf }` → `{ buf = args.buf }` に変更
