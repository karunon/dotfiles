...
export default function planModeExtension(pi: ExtensionAPI): void {
	...
	function restoreTools(): void {
		const restored = availableTools(toolsBeforePlanMode ?? FALLBACK_NORMAL_MODE_TOOLS);
		pi.setActiveTools(restored.length > 0 ? restored : FALLBACK_NORMAL_MODE_TOOLS);
		toolsBeforePlanMode = undefined;
	}

	function enablePlanMode(ctx: ExtensionContext): void {
		if (!planModeEnabled) toolsBeforePlanMode = pi.getActiveTools();
		planModeEnabled = true;
		executionMode = false;
		todoItems = [];
		const planTools = availableTools(PLAN_MODE_TOOLS);
...
Use web-research skills only if they are available in the current session.
...
After completing a step, include a [DONE:n] tag in your response.`,
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const planModeEntry = entries
			...
			.pop() as
			| { data?: { enabled: boolean; todos?: TodoItem[]; executing?: boolean; toolsBeforePlanMode?: string[] } }
			| undefined;
